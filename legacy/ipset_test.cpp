...
int ipsetAdd(std::string table, std::string set_type, uint32_t interval) {
    int32_t ret = 0;
    ...
    if (session == NULL) {
        printf(
            "ipsetAdd: Cannot initialize ipset session for "
            "set_type %s. aborting.\n",
            set_type.c_str());
        ret = 1;
    } else {
        ...
        if (ret == 0) {
            ...
            if (!type) {
                printf(
                    "ipsetAdd: Failed to get the set type from Kernel "
                    "set_type=%s table=%s sess=%p cmd=%d\n",
                    set_type.c_str(), table.c_str(), session, cmd);
                ret = 1;
            } else {
                ...
                if (ret == 0) {
                    ...
                    if (!ret) {
                        ...
                        if (ret) {
                            printf(
                                "ipsetAdd: WARNING: Failed to execute "
                                "IPSET_CMD_ADD command. ipset exists? ret=%d set_type=%s "
                                "table=%s\n",
                        ...
                        printf(
                            "ipsetAdd: Failed to set timeout option. "
                            "ret=%d set_type=%s table=%s\n",
                            ret, set_type.c_str(), table.c_str());
                    }
                } else {
                    printf(
                        "ipsetAdd: Failed to parse element. "
                        "ret=%d set_type=%s table=%s\n",
                        ret, set_type.c_str(), table.c_str());
                }
            }
            ...
            printf(
                "ipsetAdd: Failed to parse setname. "
                "ret=%d set_type=%s table=%s\n",
                ret, set_type.c_str(), table.c_str());
        }
        ...
        if (ipset_session_fini(session) != 0) {
            printf(
                "ipsetAdd: Failed to destroy ipset "
